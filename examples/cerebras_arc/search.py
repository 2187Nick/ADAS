...
log_file = f"cerebras_arc/logs/arc_search_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
visualization_log_file = f"cerebras_arc/logs/arc_visualization_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"


@backoff.on_exception(backoff.expo, Exception)
def get_json_response_from_gpt(msg, model, system_message, temperature=1.0):
    system_message += " Be concise in your thinking. Your thinking response should be exactly 1 sentence."
    
    global api_tracker
    api_tracker = api_call_with_rate_limit(api_tracker, args.max_api_calls)
    logging.info(f"API call #{api_tracker['api_call_count']} to Cerebras")
    
    try:
        #logging.info(f"System Message to Cerebras: {system_message}")
        #logging.info(f"Prompt to Cerebras: {msg}")
        ...
        logging.info(f"Response from Cerebras: {content0}")
        
        return json.loads(content0)
    
    except BadRequestError as e:
        if e.code == 'context_length_exceeded':
            logging.warning("Context length exceeded. Skipping this puzzle.")
            #return {"skip_puzzle": True}
            return {}
        ...
        if e.code == 'json_validate_failed':
            ...
            logging.warning("JSON validation failed. Attempting to fix the JSON.")
            failed_json = e.response.json().get('failed_generation', '')
            
            fix_prompt = f"The following JSON is invalid. Please fix the formatting issues and return a valid JSON object:\n\n{failed_json}"
            
            fix_response = client.chat.completions.create(
                model=model,
                messages=[
                    {"role": "system", "content": "You are a JSON formatting expert. Fix the provided JSON and return only the corrected JSON object."},
            ...
            logging.info(f"Fixed JSON response: {fixed_content}")
        ...
        logging.error(f"Error decoding Cerebras JSON")
        return {}
...
def get_json_response_from_gpt_reflect(
    ...
    global api_tracker
    api_tracker = api_call_with_rate_limit(api_tracker, args.max_api_calls)
    logging.info(f"API call #{api_tracker['api_call_count']} to Cerebras (reflect)")
    ...
    try:
        ...
        logging.info(f"Response from Cerebras (Reflect): {content0}")
        
        return json.loads(content0)
    
    except BadRequestError as e:
        if e.code == 'json_validate_failed':
            logging.warning("JSON validation failed. Attempting to fix the (Reflect) JSON.")
            failed_json = e.response.json().get('failed_generation', '')
...
class LLMAgentBase():
    ...
    def __init__(self, output_fields: list, agent_name: str,                 
            role='helpful assistant', model="llama3.1-70b", temperature=1.0) -> None:
...
def evaluate_forward_fn(args, forward_str):
    ...
    if SEARCHING_MODE:
        if args.easy_mode:
            arc_dir = "./../dataset/ARC-800-tasks/easy"
        else:
    ...
    #selected_files = all_files[0:args.num_puzzles]
...
if __name__ == "__main__":
    ...
    parser.add_argument('--save_dir', type=str, default='cerebras_arc/results/')
    parser.add_argument('--expr_name', type=str, default='arc_cerebras_results')
    ...
    parser.add_argument('--model', type=str, default='llama3.1-70b', choices=[])
    ...
    parser.add_argument('--num_puzzles', type=int, default=100, help='Number of puzzles to process')
    parser.add_argument('--max_api_calls', type=int, default=500, help='Maximum number of API calls')
    parser.add_argument('--easy_mode', type=int, default=True, help='44 Easy puzzles')
